...
# folder mode — continuous save; NOT the live-USB EXIT: buffer). PorteuX creates a
# "changes" subdirectory inside it for the AUFS overlay (see upstream porteux.cfg +
# boot/docs/cheatcodes.txt). PORTEUX_CHANGES_DIR is therefore where the installer
# pre-seeds config (it IS that overlay subdir), while PORTEUX_PERSISTENCE_DIR is
# what goes into the changes= boot parameter.
